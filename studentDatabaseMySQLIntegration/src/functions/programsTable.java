...
public class programsTable {
    ...
    public void fillTable() {
        ...
        if (connectionAttempt != null) {
            try (Statement createStatement = connectionAttempt.createStatement(); ResultSet createResult = createStatement.executeQuery(
                    "SELECT pT.programCode, pT.programName, cT.collegeName"
                    + " FROM programsTable pT"
                    + " JOIN collegesTable cT ON pT.collegeCode = cT.collegeCode"
                    + " LIMIT " + rowsPerPage
                    + " OFFSET " + (startingPage - 1) * rowsPerPage)) {
    ...
    public int searchFieldBar() {
        ...
        programsTable.setCounterLabel("Students Found: " + totalMatches);
    ...
    public int getTotalSearchPages(int rows) {
        ...
        programsTable.setTotalPageLabel(String.valueOf(totalPages));
